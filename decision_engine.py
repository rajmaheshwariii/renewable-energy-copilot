...
def dynamic_reserve_soc(
    ...
    # Risk contribution: up to +20 percentage points
    reserve += min(max(risk_score, 0.0), 100.0) * 0.20

    # Forecast uncertainty contribution: up to +15 points
    reserve += min(max(forecast_uncertainty_pct, 0.0), 50.0) * 0.30
    ...
    # Preserve extra energy if a large future shortage is expected
    if future_peak_shortage_kw > 0:
        reserve += 7.5

    return round(min(max(reserve, 15.0), 60.0), 2)
